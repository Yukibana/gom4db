...
func NewDb(rdb *gorocksdb.DB,cacheSize int) *DB {
	db := &DB{rdb: rdb}
	db.wo = gorocksdb.NewDefaultWriteOptions()
	db.ro = gorocksdb.NewDefaultReadOptions()
	db.caches = lru.New(cacheSize)
	_ = db.RawSet([]byte{MAXBYTE}, nil) // for Enumerator seek to last
	return db
}
...
func (d *DB) Del(key string) error {
	return d.RawDelete(Str2bytes(key))
}
...
func (d *DB) Set(key string, value []byte) error {
	return d.RawSet(rawKey(Str2bytes(key), STRING),value)
}
